...
namespace Diep.GameEngine.Scenario.MovableBlocks
{
    public class Stroller : Block
    {
        ...
        public override void Step(IEnumerable<object> data)
        {
            ...
            {
                if (TargetSource is Block targetBlock)
                {
        ...
        {
            int avaliableTargets = Screen.MovableShapes.Count(a => !(a is Stroller || a is Tracker) && (a as Block).Active);
            if (avaliableTargets > 0)
            {
                var target = Screen.MovableShapes.ToArray().Where(a => !(a is Stroller || a is Tracker)).OrderBy(a => Guid.NewGuid()).FirstOrDefault();
                if (target != null)
                {
