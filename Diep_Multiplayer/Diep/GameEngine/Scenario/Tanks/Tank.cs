...
namespace Diep.GameEngine.Scenario.Tanks
{
    public class Tank : IGameObject, IMovable, ITouchable, IToolSupport
    {
        ...
        public const float StandardWeight = .5f;
        ...
        public virtual void InitializeTank()
        {
            ...
            Bounds = new RectangleF(
                new PointF((float)(Extensions.Random.NextDouble() * 300), (float)(Extensions.Random.NextDouble() * 300)),
                tankSize);
            Speed = Tank.MinSpeed;
            SetCannons();
        ...
        public RectangleF Bounds
        {
            ...
            set
            {
                ...
                RepositionCannons(DrawingBounds);
            }
        }
        ...
        public virtual void RepositionCannons(RectangleF value)
        {
            if (Cannons != null)
            {
                ...
                {
                    cannon.Support = this;
                    cannon.Bounds = new RectangleF(new PointF(value.X, value.Y), cannon.Bounds.Size);
                }
            }
        }
        ...
        public RectangleF DrawingBounds
        {
            get
            {
                if (UserTank)
                {
                    var center = new PointF(Screen.Bounds.Width / 2, Screen.Bounds.Height / 2);
        ...
        {
            var da = Screen.DrawArea;
            da.Inflate(da.Width * .1f, da.Height * .1f);
            if (Bounds.IntersectsWith(da))
            {
                PerformDraw(g);
            }
        }
        ...
        protected virtual void PerformDraw(Graphics g)
        {
            ...
            //g.DrawString($"{movementVector.X} | {movementVector.Y}", new Font("Segoe UI", 8), new SolidBrush(Color.Black), DrawingBounds.GetCenter());
